...
// AddTx2Pool Add a transaction to the pool (consider the queue only)
...
// AddTxs2Pool Add a list of transactions to the pool
...
// Addtxs2poolHead add transactions into the pool head
func (txpool *TxPool) Addtxs2poolHead(tx []*Transaction) {
	txpool.lock.Lock()
	defer txpool.lock.Unlock()
...
func (txpool *TxPool) PackTxs(maxTxs uint64) []*Transaction {
	txpool.lock.Lock()
	defer txpool.lock.Unlock()
	txNum := maxTxs
...
// PackRelayTxs abort ! Pack relay transactions from relay pool
...
// TransferTxs abort ! Transfer transactions when re-sharding
