...
func (p *ConsensusNode) set2DMap(isPrePareConfirm bool, key string, val *shard.Node) {
	if isPrePareConfirm {
...
func (p *ConsensusNode) writeCsvLine(metricName []string, metricVal []string) {
	// Construct directory path
	dirPath := params.DataWritePath + "PbftShardNum=" + strconv.Itoa(int(p.pbftChainConfig.ShardNums))
